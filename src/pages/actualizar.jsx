...
export function EditarProducto() {
  ...
  // useEffect para cargar los datos del producto al montar el componente 
  useEffect(() => {
    (async () => {
  ...
  // useEffect para actualizar los datos del formulario cuando el producto esté disponible
  useEffect(() => {
  ...
  // Manejo de la acción de guardar cambios
  const handleSubmit = async () => {
    ...
    // Llamada a la función del contexto para editar
    await editarProduct(id, datosActualizados);

    // Alerta de confirmación con estilo
    Swal.fire({
      title: 'Modificado',
  ...
  // Si aún está cargando, mostramos componente de carga
  if (loading) return Cargando();

  // Si ya hay datos, renderizamos el formulario
  if (producto.detalles)
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-gray-950 text-white flex items-center justify-center px-4 py-20">
        <form
          onSubmit={(e) => e.preventDefault()} // Evita recarga de página
